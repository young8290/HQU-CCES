...
export async function getScoresByClass(classId: number, academicYearId: number) {
  ...
  return students.map((student) => {
    const scoreMap: Record<string, { value: number; remark: string | null }> = {};
...
export async function getScoresByStudent(studentId: number, academicYearId: number) {
...
async function recalculateSportsTotal(studentId: number, academicYearId: number, updatedBy?: number) {
  ...
  const sportsBase = scores.find((s) => s.category === 'sports_base')?.value || 0;
  const sportsReward = scores.find((s) => s.category === 'sports_reward')?.value || 0;
